...
export const ProgressProvider = ({ children }) => {
  ...
  const [stepPhase, setStepPhase] = useState('stop'); // 'stop' | 'thinking' | 'answering' | 'done'
  ...
  useEffect(() => {
    if (step === null || !interview.data[step]) return;
    goToStep(step);
  }, [step]);
  ...
  const goToStep = useCallback(number => {
    //if (!interview || !interview.data || !interview.data[number]) return;
    const { text, type } = interview?.data[number];
    console.log(text);
    setStepPhase('thinking');
    startCountdown(15, () => setStepPhase('stop'), setCountdown);
  }, []);
  ...
  //   if (!text || step >= interview.data.length) return;
  ...
  //   if (step < interview.data.length - 1) {
